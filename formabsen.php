...
if(isset($_POST['submit'])){
    ...
    if(!empty(trim($nim)) && !empty(trim($id_mk))){
        if($nim == $sesi){
            ...
            if($result){
            echo "<script>alert('Absen BERHASIL dilakukan.');window.location='login.php';</script>";
            session_unset();
            session_destroy();
            exit;
        }else{
            echo "<script>alert('Absen GAGAL dilakukan.');window.location='formabsen.php';</script>";
            }
        }else{
            echo "<script>alert('GAGAL yang dimaksukkan invalid');window.location='formabsen.php';</script>";
        }
    
    }else{
        echo "<script>alert('Data tidak boleh kosong!');window.location='formabsen.php';</script>";
    }
}
?>
<html>
...
<div class="background">
    ...
    <form method="POST" action="formabsen.php">
        ...
        <br><br>
        <p>Tidak bisa absen? <a href="404page.php">Hubungi Admin (Bapendik)</a></p>
        <!-- <div class="social">
